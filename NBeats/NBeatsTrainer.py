...
class NBeatsTrainer:
  ...
  def getModelName(self, _name_backcast, _name_epoch, _name_loss):
    return self.prefix + '_' + _name_backcast + '_' + _name_epoch + '_' + _name_loss
  ...
  def train_epoch(self, _epoch, _x, _y, _net, _optimizer, _name_backcast=NAME_BACKCAST_3H, _name_epoch=NAME_EPOCH_5K, _name_loss=NAME_LOSS_MAPE, _batch_size=256, _print_epoch=100, _shuffle=True, _save=True):
    ...
    for __x, __y in __data_loader:
      ...
      _optimizer.step()
    __train_epoch += 1
    if (__train_epoch > 0) & (_save):
      with torch.no_grad():
        self.save(_net, _optimizer, __train_epoch, _name_backcast, _name_epoch, _name_loss)
    if (_epoch > 0) & (__train_epoch % _print_epoch == 0):
      print(f'Train - Step = {str(__train_epoch).zfill(6)}, loss = {__loss.item():.6f}')
      ...
      for __name_backcast in __list_backcast:
        ...
        for __name_loss in __list_loss:
          ...
          for __epoch in range(DICT_EPOCH[__name_epoch]):
            self.train_epoch(__epoch, __train_x, __train_y, __net, __optimizer, __name_backcast, __name_epoch, __name_loss, _batch_size)
