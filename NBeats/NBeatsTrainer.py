...
DICT_EPOCH = {
    NAME_EPOCH_5K: 5000,
    NAME_EPOCH_10K: 10000,
    NAME_EPOCH_15K: 15000,
    NAME_EPOCH_50K: 50000,
    NAME_EPOCH_150K: 150000,
}
DICT_BACKCAST = {
    NAME_BACKCAST_2H: 2,
    NAME_BACKCAST_3H: 3,
    ...
    NAME_BACKCAST_8H: 8,
    NAME_BACKCAST_9H: 9,
    NAME_BACKCAST_10H: 10,
    NAME_BACKCAST_24H: 24,
}
DICT_LOSS_FUNCTION = {
    NAME_LOSS_MAPE: operators.calcLossMAPE,
    NAME_LOSS_MSE: operators.calcLossMSE,
    NAME_LOSS_MASE: operators.calcLossMASE,
    NAME_LOSS_PA: operators.calcLossPA,
    NAME_LOSS_RMSE: operators.calcLossRMSE,
    NAME_LOSS_SMAPE: operators.calcLossSMAPE,
}
DICT_ENSEMBLE = {
...
class NBeatsTrainer:
    ...
    def printPlot(self, _x, _y, _forecast):
        ...
        for __plot_id, __idx in enumerate(np.random.choice(range(len(_x)), size=4, replace=False)):
            x = _x[__idx][3]
            __plot_x, __plot_y, __plot_forecast = (
                _x[__idx].reshape([-1, 1]).cpu().detach().numpy(),
                _y[__idx].cpu().detach().numpy(),
                _forecast[__idx].cpu().detach().numpy(),
            )
            ...
            plt.plot(
                np.arange(__len_backcast - 1, __len_backcast + __len_forecast - 1),
                __plot_y,
                color="g",
            )
            plt.plot(
    ...
    def getModelName(self, _name_backcast, _name_epoch, _name_loss, _is_best=False):
        __model_file_name = (
            self.prefix + "_" + _name_epoch + "_" + _name_backcast + "_" + _name_loss + "_" + str(self.device) + ".ckpt"
        )
        if _is_best:
    ...
    def getNet(self, _len_forecast, _len_backcast):
        ...
        __optimizer = optim.SGD(__net.parameters(), lr=0.001)
    ...
    def evaluation(
        ...
        _net,
        _optimizer,
        _name_backcast=NAME_BACKCAST_3H,
        _name_epoch=NAME_EPOCH_5K,
        _name_loss=NAME_LOSS_MAPE,
        _print_plot=False,
    ):
        with torch.no_grad():
            ...
            if _print_plot:
                ...
                print(
                    f"Evaluation - Name = {str(__model_name)}, loss = {__loss.item():.6f}, RMSE = {__loss_rmse.item():.6f}"
                )
            if __loss < self.global_loss:
                print(
                    f"Refesh Best Model - Name = {str(__model_name)}, loss = {self.global_loss:.6f} -> {__loss.item():.6f}"
                )
    ...
    def train_epoch(
        ...
        __loss = torch.tensor(0.0).to(self.device)
        _net.train()
        __list_loss = []
    ...
    def train(
        ...
        _len_forecast,
        _batch_size=1024,
        _name_ensemble_set=NAME_ENSEMBLE_SET_SMALL,
    ):
        ...
        for __name_epoch in __list_epoch:
            for __name_backcast in __list_backcast:
                ...
                __train_x, __train_y, __train_idx = NBeatsDatasetMaker.makeDataset(
                    _data_train, __len_backcast, _len_forecast, self.device
                )
                __eval_x, __eval_y, __eval_idx = NBeatsDatasetMaker.makeDataset(
                    _data_eval, __len_backcast, _len_forecast, self.device
                )
    ...
    def train_direct(
        ...
        _eval_y,
        _len_forecast,
        _name_backcast=NAME_BACKCAST_7H,
        _name_epoch=NAME_EPOCH_50K,
        _name_loss=NAME_LOSS_MSE,
        _batch_size=1024,
    ):
        self.global_loss = 100000
        ...
        for __epoch in range(DICT_EPOCH[_name_epoch]):
            __train_step, __loss_train = self.train_epoch(
                __train_step,
                __train_x,
                __train_y,
                __net,
