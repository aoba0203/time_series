...
DICT_LOSS_FUNCTION = {NAME_LOSS_MAPE: operators.calcLossMAPE, NAME_LOSS_MSE: operators.calcLossMSE, NAME_LOSS_MASE: operators.calcLossMASE, NAME_LOSS_PA:operators.calcLossPA, NAME_LOSS_RMSE:operators.calcLossRMSE}
DICT_ENSEMBLE = {
...
#       self.makePlotScatter(np.arange(__len_backcast, __len_backcast + __len_forecast), __plot_forecast, color='r')
  ...
  def train_direct(self, _train_x, _train_y, _eval_x, _eval_y, _len_forecast, _name_backcast=NAME_BACKCAST_7H, _name_epoch=NAME_EPOCH_50K, _name_loss=NAME_LOSS_MSE, _batch_size=1024):
    ...
    for __epoch in range(DICT_EPOCH[_name_epoch]):
      __train_step = self.train_epoch(__train_step, __train_x, __train_y, __net, __optimizer, _name_backcast, _name_epoch, _name_loss, _batch_size)
      __scheduler.step()
    ...
    self.evaluation(__eval_x, __eval_y, __net, __optimizer, _name_backcast, _name_epoch, _name_loss, _print_plot=True)
    print('------ END Train ------')
